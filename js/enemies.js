...
export function createEnemies(scene) {
    ...
    const enemySpacingX = scene.game.config.width * 0.1; 
    // Espaço entre os inimigos (10% da largura)
    const enemySpacingY = scene.game.config.height * 0.08; 
    // Espaço entre linhas

    const startX = (scene.game.config.width - (enemyCols - 1) * 
    enemySpacingX) / 2; // Centraliza os inimigos
    ...
    for (let row = 0; row < enemyRows; row++) {
        for (let col = 0; col < enemyCols; col++) {
            ...
            ).setScale(scene.game.config.width * 0.05 / 48); 
            // Ajusta tamanho
...
export function updateEnemies(scene, time) {
    if (time > moveTimer) {
        let edgeHit = false; 
        // Verifica se os inimigos atingiram a borda
        ...
        moveTimer = time + 500; 
        // Controla a velocidade da movimentação
    }
}
